...
import uk.ac.liverpool.narrative.StoryWorld;
	...
	public static void main(String[] args) {
		...
		try {
			...
			String pr= "(define (problem templateProblem) (:domain tempdom)\r\n" + 
					...
					"";
			URL url = new URL(
					"http://10.0.0.92:8080/NarrativeWebApp/narrative/postJson?asynch=true");
			...
			sw.domain = dom;
			sw.problem = new String[]{pr};
//			sw.template = tmp;
			OutputStream outputStream = connection.getOutputStream ();
			outputStream.write("?asynch=true&".getBytes());
			m.writeValue(outputStream, sw);
			outputStream.flush();
			outputStream.close();
			...
			List<StringSolution> s = m.readValue(sb.toString(), new TypeReference<List<StringSolution>>(){
			});
			for( StringSolution c:s) {
				System.out.println(c.actions.get(0));
			}
