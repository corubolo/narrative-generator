...
public abstract class STRIPSTranslator
{
	...
	{
		File stripsPFile = new File(name);
		...
		try
		{
			...
			pfileBufWriter.write("(define (problem "+stripsPFile.getName().replace(".", "_")+") (:domain "+gproblem.name+")\n");
			//write out types
			pfileBufWriter.write("(:objects \n");
			
			Set<Parameter> params = new SetWrapper<Parameter>();
			for (Object o : gproblem.actions)
			{
					params.addAll(((Action)o).parameters);
			}
			...
			for (Object io: gproblem.initial)
			{
			...
			for (Object go : gproblem.goal.getFacts())
			{
	...
	{
		Set stripsActions = new SetWrapper(complexGP.actions);
		Set stripsInitial = new SetWrapper();
		And stripsGoal = new And();
		Map stripsFunctions = new MapWrapper();
	...
	/**
	 * Convert a complex domain and problem into a STRIPS representation suitable for SAS+ translation.
	 * @param domain
	 * @param pfile
	 * @return
	 */
	...
	/**
	 * Splits an ungrounded problem into a STRIPS representation suitable for SAS+ translation.
	 * @param domain
	 * @param pfile
