...
public class PDDL21parser implements PDDL21parserConstants {
    ...
    public static UngroundProblem parseStreams(Reader domain, Reader problem) throws ParseException, IOException
        {
                reset();
                PDDL21parser parser = new PDDL21parser(domain);
                boolean probsuc = false;
                boolean suc = parser.parseDomain();
                domain.close();
                if (suc) {
                        parser = new PDDL21parser(problem);
                        probsuc =  parser.parseProblem();
                        problem.close();
                }
                if (probsuc)
                {
  ...
  final public boolean parseRequireDef() throws ParseException {
    ...
    while (true) {
      ...
      if (jj_2_8(20)) {
        ...
        System.out.println("cannnot parse "+t.toString().toLowerCase()); {if (true) return false;}
      } else {
        jj_consume_token(-1);
...
// **********************************
  final public void parseConstraints() throws ParseException {
        List<GroundFact> e = new LinkedList<GroundFact>();
        Fact c;
    jj_consume_token(OPEN);
...
// *	Parse Sometime
// **********************************
  final public void parseSometime() throws ParseException {
        Fact e;
    if (jj_2_99(20)) {
      jj_consume_token(OPEN);
      ...
      jj_consume_token(CLOSE);
                                                                                                           UP.sometimes.add(and);
    } else if (jj_2_100(20)) {
      jj_consume_token(OPEN);
      jj_consume_token(SOMETIMEBEFORE);
                                And and = new And();
      label_20:
      while (true) {
        if (jj_2_96(20)) {
      ...
      jj_consume_token(CLOSE);
                                                                                                                 UP.sometimesb.add(and);
    } else if (jj_2_101(20)) {
      jj_consume_token(OPEN);
      jj_consume_token(SOMETIMEAFTER);
                               And and = new And();
      label_21:
      while (true) {
        if (jj_2_97(20)) {
...
// **********************************
  ...
  final public int parseOptimization() throws ParseException {
    ...
    t1 = jj_consume_token(OPTIMIZATION);
                if (t1.toString().toLowerCase().equals("minimize")) {if (true) return Metric.MINIMIZE;}
                else if (t1.toString().toLowerCase().equals("maximize")) {if (true) return Metric.MAXIMIZE;}
  ...
  private boolean jj_3_101() {
    if (jj_scan_token(OPEN)) return true;
    if (jj_scan_token(SOMETIMEAFTER)) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
  ...
  private boolean jj_3_100() {
    if (jj_scan_token(OPEN)) return true;
    if (jj_scan_token(SOMETIMEBEFORE)) return true;
    Token xsp;
    while (true) {
      xsp = jj_scanpos;
