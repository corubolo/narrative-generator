 ...
 *
 */
//Welcome to the most infuriating type hierarchy ever designed. Why Keith? WHY?!?!
public class ForAll implements GroundFact, UngroundFact, CompoundLiteral, Quantifier, ADLFact
{
	...
	protected ForAll(Variable v, Fact c, boolean grounded)
	{
		...
		this.objects = new SetWrapper<PDDLObject>();
	}
...
//		return true;
	...
	{
//		return this;
		return new ForAll(this.variable, ((GroundFact)this.condition).staticify(fValues), true);
...
//	 * literals into individual ones. Individual literals/conjunctions are returned wrapped in an
	...
	{
		Set<Fact> groundedFacts = new SetWrapper<Fact>();
		And compiledOut = new And();
		for (PDDLObject v : this.objects)
		{
			if (varMap.containsValue(v))
			{
				//o hai! I see you want to work with the same var twice in a single predicate. 
			...
			Map<Variable, PDDLObject> newMap = new MapWrapper<Variable, PDDLObject>(varMap);
	...
	public Collection<? extends STRIPSFact> toSTRIPS()
	{
		...
		{
			Collection<STRIPSFact> set = new SetWrapper<STRIPSFact>();
	...
	{
//		Set<SingleLiteral> s = new SetWrapper<SingleLiteral>();
