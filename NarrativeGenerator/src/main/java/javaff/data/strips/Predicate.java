...
public class Predicate extends Literal implements UngroundFact, SingleLiteral
{
	...
	public Proposition ground(Map<Variable, PDDLObject> varMap)
	{
		...
		Map<String, PDDLObject> sarMap = new MapWrapper<String, PDDLObject>();
		for (Entry<Variable, PDDLObject> e: varMap.entrySet()) {
			sarMap.put(e.getKey().getName(), e.getValue());
		}
		...
		for (Parameter o : this.parameters)
		{
			...
			else
			{
				...
				po = (PDDLObject) sarMap.get(v.getName());
				if (po == null)
